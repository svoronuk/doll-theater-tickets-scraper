...
import { showImages, formatShowMessage, formatShowListMessage } from './notifications';
import { loadMonths, saveMonths } from './months';
import { checkSelectorsHealth } from './services/health';
import { config } from './config/config';

// Add a lock to prevent concurrent executions
let isCheckingWebsite = false;
...
export async function checkWebsite(): Promise<void> {
	// If already checking, return immediately to prevent duplicate runs
	if (isCheckingWebsite) {
		console.log('checkWebsite: already running, skipping this call');
		return;
	}

	// Set the lock
	isCheckingWebsite = true;
	...
	try {
		...
		if (config.debugMode) {
			...
			let latestMonthId =
				storedMonths.length > 0 ? storedMonths[storedMonths.length - 1] : null;
			...
			const nextMonthDate = latestMonthId
				? addMonths(parseMonth(latestMonthId), 1)
				: new Date();
